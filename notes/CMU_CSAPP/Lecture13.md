...
所以，尽可能避免全局变量。如果需要，尽可能使用static。定义时记得初始化。如果使用外部全局变量，用extern属性告诉编译器。

#### Relocation

在重定向中，链接器会将目标文件的所有代码放在每个模块的`.text`部分，并连续的存储在可执行文件的`.text`部分。可执行文件包含所有系统代码的模块定义的代码，并对`.data`做相同的事情。之后还有符号表和调试信息。
