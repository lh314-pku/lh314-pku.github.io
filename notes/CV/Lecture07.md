...
当图像分辨率很大的时候，$x$ 和 $y$ 会很大，矩阵 $U$ 中的参数往往在 $10^3-10^6$，我们需要对关键点坐标进行中心化：取其平均坐标作为中心点，把中心点平移到坐标原点：
...
而后进行放缩，调整点的分布范围，使所有点到原点的平均距离为某个标准值
...
这样讲坐标进一步更新为 $(sx_i,sy_i)$，并得到归一化矩阵（在齐次坐标下）：
...
求得两幅图片的归一化矩阵 $T$ 和 $T'$ 后，在归一化后坐标下计算 $F$，再进行逆变换：
